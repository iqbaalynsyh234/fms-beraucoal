...
<body>
    ...
    function fetchDataAndShowData() { // show data
        $.ajax({
            url: '<?= base_url() ?>streetdrawing/label', // Update the URL to the actual endpoint
            method: 'GET',
            dataType: 'json',
                ...
                if (response.success) {
                    ...
                    for (var i = 0; i < geofenceData.length; i++) {
                        var style_green = {
                            ...
                            fillOpacity: 0.4,
                            fontSize: '12px',
                            label: geofenceData[i].street_name
                        };
                        vectors.style = style_green;
                        deserialize(geofenceData[i].street_serialize);
                    }
                } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    var popupContent = '<h3>Street List</h3>';
                    for (var i = 0; i < geofenceData.length; i++) {
                        popupContent += '<p>' + geofenceData[i].street_name + '</p>';
                    }
    ...
    jQuery(document).ready(function() {
        ...
        var style_green = {
            ...
            fillOpacity: 0.4,
            fontSize: '12px',
            label: '<?php echo $geofence[$i]->street_name; ?>'
        };
        vectors.style = style_green;
        deserialize('<?php echo $geofence[$i]->street_serialize; ?>');
        <?php } ?>
    ...
    <?php 
		...
		//$key = "AIzaSyDJCuzzQJJ7QHwAc6QoCoQXO4X1qQr54f0";
	?>
    ...
    <script type="text/javascript"
        src="https://maps.googleapis.com/maps/api/js?key=<?php echo $key;?>&libraries=drawing"></script>
        ...
        <div class="page-content">
            <div class="panel" id="panel_form">
                ...
                <fieldset>
                    ...
                    <a href="<?=base_url();?>streetdata" class="btn btn-flat" type="button">Street List</a>
                    <input class="button" type="button" name="btncancel" id="btncancel"
                        ...
                        onclick="location='<?= base_url() ?>streetdrawing/manage/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>/<?php echo uniqid(); ?>'; " />
                    <?php } ?>
                </fieldset>



                <header class="panel-heading panel-heading-green">Street Drawing Polygon</header>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered mw-90">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title" id="exampleModalLabel">List Street Name</h4>
...
function InitMap(koordinatArrayAll) {
    ...
    function tampilkanListCreatePolygonUI() {
        ...
        for (var i = 0; i < listCreatePolygon.length; i++) {
            ...
            modalBody.innerHTML += '<tr>' +
                '<th scope="row">' + (i + 1) + '</th>' +
                '<td><input type="text" class="form-control" id="geofenceNameInput_' + i + '" value="' + geofenceName +
                '"></td>' +
    ...
    for (var i = 0; i < koordinatArrayAll.length; i++) {
        ...
        var marker = new google.maps.Marker({
            ...
            map: map,
            label: {
                text: koordinatArrayAll[i][0].street_name,
                color: 'black',
                fontWeight: 'normal'
            },
            icon: {
...
function simpanData(listCreatePolygon) {
    ...
    $.post('<?= base_url() ?>streetdrawing/save', postData)
...
function showdata() {
    ...
    jQuery.post('<?php echo base_url(); ?>streetdrawing/label', {
            deviceid: deviceid
        },
        function(r) {
            if (r.success) {
...
function save(str) {
    jQuery.post(
        "<?= base_url() ?>streetdrawing/manage/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>", {
            json: str
        },
        function(r) {
            if (r.error) {
...
function removegeofence() {
    ...
    jQuery.post(
        ...
        function(r) {
            ...
            location =
                '<?= base_url() ?>streetdrawing/manage/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>/<?php echo uniqid(); ?>';
        }, "json"
    );
...
function othervehicle(elmt) {
    ...
    location = '<?= base_url() ?>streetdrawing/manage/' + v;
}
