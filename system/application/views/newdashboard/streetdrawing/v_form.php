...
<script>
	...
	function frmadd_onsubmit(frm)
	{
		...
		location = '<?=base_url()?>streetdataview';
	...
	function removegeofence()
	{
		...
		jQuery.post("<?=base_url()?>geofencedatalive/remove2/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>", {},
			function(r)
			{
				if (r.error)
				{
				...
				location = '<?=base_url()?>geofencedatalive/managestreet/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>/<?php echo uniqid();?>';
			}
			, "json"
		);
