...
<script>
  ...
  var intervalkmlist, current_totalov;
  intervalkmlist          = setInterval(data_list_first, 3000);
  var simultantype        = 0;
  ...
  function data_list_first(){
    ...
    jQuery("#loader2").show();
    $.post("<?php echo base_url() ?>violation/getdatalisthistorikalnew", data, function(response){
      jQuery("#loader2").hide();
      ...
      if (response.violationmix.length > 0) {
          ...
          var htmltable = "";
          for (var i = 0; i < violationmix.length; i++) {
            var violation = violationmix[i].violation;
              if (violation.includes("Fatigue")) {
                ...
                htmltable += '<tr style="background-color:white; color:black;">';
              }
