        ...
        <div class="row">
            <div class="col-md-12 col-sm-12">
                <div class="panel" id="panel_form">
                    <header class="panel-heading" id="violation-historikal">Violation Historikal Table Report</header>
                    <div class="panel-body" id="bar-parent10">
                        <form class="form-horizontal form" name="frmsearch" id="frmsearch"
                            onsubmit="javascript:return frmsearch_onsubmit()">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-4 col-md-3">
                                    <select class="form-control select2" id="company" name="company"
                                        ...
                                        <?php
                                        $ccompany = count($company);
                                        for ($i = 0; $i < $ccompany; $i++) {

                                          echo "<option value='" . $company[$i]->company_id . "'>" . $company[$i]->company_name . "</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
                            ...
                            <div class="form-group row" id="mn_vehicle">
                                <label class="col-lg-3 col-md-3 control-label">Violation
                                </label>
                                <div class="col-lg-4 col-md-3">
                                    <select id="violationmasterselect" name="violationmasterselect"
                                        class="form-control select2">
                                        <option value="0">--All Violation</option>
                                        <?php
                      for ($i = 0; $i < sizeof($violationmaster); $i++) {?>
                                        <option value=<?php echo $violationmaster[$i]["alarmmaster_id"] ?>>
                                            <?php echo $violationmaster[$i]["alarmmaster_name"] ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="text-right">
                                <button class="btn btn-success btn-circle" id="btnsearchreport"
                    ...
                    <div id="loader2" style="display:none;"
                        class="mdl-progress mdl-js-progress mdl-progress__indeterminate is-upgraded"
...
function page(p) {
    ...
    jQuery("#loader2").show();
    jQuery("#result").hide();
    jQuery.post("<?= base_url(); ?>violation/search_violationtablehistorikal", jQuery("#frmsearch").serialize(),
        function(r) {
...
function company_onchange() {
    ...
    if (data_company == 0) {
        ...
        jQuery("#vehicle").html(
            "<option value='all' selected='selected' class='form-control'>--All Vehicle--</option>");
    } else {
        jQuery("#mn_vehicle").show();

        var site = "<?= base_url() ?>violation/get_vehicle_by_company_with_vdevice/" + data_company;
        jQuery.ajax({
            url: site,
