          ...
          <?php $no = 1; for ($i=0; $i < sizeof($content); $i++) {
            ...
            <?php
              $alarm_report_intervention_category_cr = $content[$i]['alarm_report_intervention_category_cr'];
                ...
                <button type="button" class="btn btn-warning" id="btn_intervention" onclick="modal_intervention_controlroom_p1('<?php echo $content[$i]['alarm_report_vehicle_id'].','.$content[$i]['alarm_report_start_time'].','.$content[$i]['alarm_report_id'].','.$content[$i]['alarm_report_type'].','.$content[$i]['alarm_report_imei']; ?>');" title="Intervensi">
