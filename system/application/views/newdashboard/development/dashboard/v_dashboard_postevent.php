...
<script type="text/javascript">
  ...
  function frmsearch_onsubmit(){
    ...
    $("#loadernya").show();
    $.post("<?php echo base_url() ?>development/searchreport_postevent", jQuery("#frmsearch").serialize(), function(response){
      if (response.error) {
  ...
  function modal_post_event(id){
   $("#loadernya").show();
    ...
    $.post("<?php echo base_url() ?>development/postevent_detail", {alert_id : data[0], sdate : data[1], alarm_report_id : data[2], alarmtype : data[3]}, function(response){
    $("#loadernya").hide();
