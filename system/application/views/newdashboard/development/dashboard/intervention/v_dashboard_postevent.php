        ...
        <div class="panel" id="panel_form">
          ...
          <div class="panel-body" id="bar-parent10">
            <form class="form-horizontal" name="frmsearch" id="frmsearch" onsubmit="frmsearch_onsubmit();">
              ...
              <input type="text" id="time_alert" hidden>
...
<script type="text/javascript">
  ...
  var interval_intervention = setInterval(frmsearch_onsubmit, 30000);
  ...
  function frmsearch_onsubmit(){
    ...
    $("#loadernya").show();
    $.post("<?php echo base_url() ?>development/search_intervention_existing", jQuery("#frmsearch").serialize(), function(response){
      if (response.error) {
  ...
  function getdetailinfo(id){
    ...
    $.post("<?php echo base_url() ?>development/getinfodetail_new_existing", {alert_id : data[0], sdate : data[1]}, function(response){
	  $("#loadernya").hide();
      console.log("response getdetailinfo: ", response);
      $("#modalStateTitle").html("Detail Information - DEVELOPMENT EXISTING");
  ...
  function modal_post_event_controlroom(id){
   $("#loadernya").show();
    ...
    $.post("<?php echo base_url() ?>development/intervention_detail_existing", {alert_id : data[0], sdate : data[1], alarm_report_id : data[2], alarmtype : data[3]}, function(response){
    $("#loadernya").hide();
  ...
  //   console.log("sdate detail : ", data[1]);
  //
  //   $.post("<?php echo base_url() ?>dashboardberau/getinfodetail", {alert_id : data[0], sdate : data[1]}, function(response){
	//   $("#loadernya").hide();
  //     console.log("response getdetailinfo: ", response);
  //     $("#contentinformationdetail").html(response.html);
  //     $("#modalinformationdetail").show();
  ...
  // }
    ...
    function start_stop_refresh(v)
  		{
  			switch(v)
  			{
  				case "stop":
  					jQuery("#btn_stop_interval").hide();
  					jQuery("#btn_start_interval").show();
            clearInterval(interval_intervention);
  				break;
  				case "start":
          jQuery("#btn_stop_interval").show();
          jQuery("#btn_start_interval").hide();
          interval_intervention = setInterval(frmsearch_onsubmit, 10000);
  				break;
  			}
  		}
