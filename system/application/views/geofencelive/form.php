...
<script>
	...
	function frmadd_onsubmit(frm)
	{
		...
		location = '<?=base_url()?>geofencelive/manage/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>/label/<?php echo uniqid();?>';
		return false;
	}

	function save(str)
	{
		jQuery.post("<?=base_url()?>geofencelive/save/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>", {json: str},
			function(r)
			{
				if (r.error)
				{
	...
	function removegeofence()
	{
		...
		jQuery.post("<?=base_url()?>geofencelive/remove/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>", {},
			function(r)
			{
				if (r.error)
				{
				...
				location = '<?=base_url()?>geofencelive/manage/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>/<?php echo uniqid();?>';
			}
			, "json"
		);
	...
	{
		showdialog();
		jQuery.post('<?php echo base_url(); ?>geofencelive/label', {deviceid: '<?php echo $vehicle->vehicle_device; ?>'},
			function(r)
			{
	...
	{
		jQuery.post("<?php echo base_url(); ?>geofencelive/get/"+gid, {},
			function(r)
			{
	...
	function removegeofence(gid)
	{
		...
		jQuery.post("<?php echo base_url(); ?>geofencelive/removebyid/"+gid, {},
			function(r)
			{
				if (r.error)
				{
	...
	function removegeofence_byvehicle222(gid)
	{
		if (! confirm("<?php echo $this->lang->line('lconfirm_delete'); ?>")) return;

		jQuery.post("<?php echo base_url(); ?>geofencelive/removebyvehicle/"+gid, {},
			function(r)
			{
				if (r.error)
				{
	...
	function savegeo()
	{
		jQuery.post("<?php echo base_url(); ?>geofencelive/savelabel/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>", jQuery("#frmadd1").serialize(),
			function(r)
			{
	...
	{
			showdialog();
			jQuery.post('<?php echo base_url(); ?>geofencelive/copyto', {vid: <?php echo $vehicle->vehicle_id ?>},
				function(r)
				{
					if (r.error)
					{
	...
	function gotovehicle()
	{
		...
		function othervehicle(elmt)
		{
			...
			location = '<?=base_url()?>geofencelive/manage/'+v;
		}
	...
	var map, vectors, kml_tracker5 = null;
        ...
        <h1>GEOFENCE SETTING TES (DBLive)</h1>
        ...
        <!--<input class="button" type="button" name="btncopy" id="btncopy" value=" <?php echo $this->lang->line("lgeofence_copy_to"); ?> " onclick="javascript:copyto();" />-->
		...
		<input class="button" type="button" name="btncancel" id="btncancel" value=" <?php echo $this->lang->line("lcenter_to_geofence"); ?> " onclick="location='<?=base_url()?>geofencelive/manage/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>/<?php echo uniqid();?>';" />
        </fieldset>
        </td>
        ...
        <legend>Control</legend>
            <input class="button" type="button" name="btnsave" id="btnsave" value=" Save " onclick="javascript: frmadd_onsubmit(this)" />
            <input class="button" type="button" name="btnlabel" id="btnlabel" value=" <?php echo $this->lang->line("lgeofence_list"); ?> " onclick="javascript:showdata()" />
            <input class="button" type="button" name="btncancel" id="btncancel" value=" <?php echo $this->lang->line("lreset"); ?> " onclick="location='<?=base_url()?>geofencelive/manage/<?php echo $this->uri->segment("3"); ?>/<?php echo $this->uri->segment("4"); ?>/<?php echo uniqid();?>';" />
        </fieldset>
        </td>
        </tr>
        </table>
