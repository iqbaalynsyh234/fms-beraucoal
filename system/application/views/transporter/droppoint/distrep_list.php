...
function page(p) {
    ...
    jQuery("#offset").val(p);
    jQuery("#loader").show();
    jQuery.post("<?=base_url();?>droppoint/search_distrep/" + p, jQuery("#frmsearch").serialize(),
        function(r) {
            jQuery("#loader").hide();
...
function add() {
    showdialog();
    jQuery.post('<?=base_url()?>droppoint/add_distrep/', {},
        function(r) {
            if (r.error) {
                alert("Retry");
                return;
            ...
            showdialog(r.html, 'Form Add Distrep');
        }, "json"
    );
}
...
function edit(id) {
    showdialog();
    jQuery.post('<?=base_url()?>droppoint/edit_distrep/' + id, {},
        function(r) {
            if (r.error) {
                alert("Retry");
                return;
            ...
            showdialog(r.html, 'Form Edit Distrep');
        }, "json"
    );
}
...
function delete_data(id) {
    if (confirm("Are you sure delete this data?")) {
        jQuery.post('<?=base_url()?>droppoint/delete_distrep/' + id, {}, function(r) {
            if (r.error) {
                alert(r.message);
                return;
            } else {
...
<div style="position: absolute; margin: 0; padding: 0; z-index: 1000; width: 100%;">
    ...
    <div id="main" style="margin: 20px;">
        <div class="block-border">
            <form name="frmsearch" class="block-content form" id="frmsearch"
                ...
                <fieldset class="grey-bg required">
                    ...
                    <table width="100%" cellpadding="3" class="tablelist">
                        ...
                        <tr>
                            <td>
                                <select id="field" name="field" onchange="javascript:field_onchange()">
                                    <option value="distrep_name">Name</option>
                                    <option value="distrep_code">Code</option>
                                </select>

                                <input type="text" name="keyword" id="keyword" value="" class="formdefault" />
                                <input type="text" name="cp" id="cp" value="" class="formdefault"
                                    style="display: none;" />
						...
						To <input type='text' name="enddate" id="enddate"  class="date-pick" value=""  maxlength='10'>
                                ...
                                <img id="loader" src="<?=base_url();?>assets/images/ajax-loader.gif" border="0" alt=""
