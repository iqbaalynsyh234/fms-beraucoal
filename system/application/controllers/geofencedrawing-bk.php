...
class geofencedrawing extends Base
{
	...
	function label()
	{
		...
		// 				$lng = $coordinates[0];
		// 				$lat = $coordinates[1]; // Use index 1 for latitude
		// 				$benar[] = array(
		// 					'lat' => floatval($lat),
		// 					'lng' => floatval($lng),
		// 				);
	...
	function showpopup()
	{
		...
		foreach ($data as $row) {
			...
			foreach ($array as $key => $value) {
				if ($key === 8) {
					// Skip this index, as per your original code
				} else {
					$coordinates = explode(',', $value);
					$geofence_name = $row->geofence_name;
					$lng = $coordinates[0];
		...
		$callback['koordinat_array_all'] = $koordinat_array;
		echo json_encode($callback);
		return view('geofence/label');
