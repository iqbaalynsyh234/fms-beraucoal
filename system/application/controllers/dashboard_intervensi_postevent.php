...
class Dashboard_intervensi_postevent extends Base
{
...
function dashboardintervention(){
  ...
  if ($privilegecode == 5 || $privilegecode == 6) {
    $data_karyawan_bc                    = $this->m_dashboard_intervensi_postevent->data_karyawan_bymitra($data_company[0]->company_sid_code);
  }else {
  ...
  if ($privilegecode == 1) {
    ...
    $this->params["content"]        = $this->load->view('newdashboard/dashboardberau/intervention/part_1/v_dashboard_intervention', $this->params, true);
...
function check_isinput_intervention_config(){
  ...
  $intervention_config = $this->m_dashboard_intervensi_postevent->check_intervention_config_2($user_company, $sdate_config, $edate_config);
...
function intervention_detail_controlroom_p1(){
  ...
  if ($alarmtype == "Overspeed") {
    ...
    $data_karyawan_bc                         = $this->m_dashboard_intervensi_postevent->data_karyawan_bymitra_jabatan($company[0]->company_sid_code, $jabatan_search);
    ...
    $data_location                            = $this->m_dashboard_intervensi_postevent->master_location_bc("ts_bc_master_location", $location_id_sync);
...
function submit_intervention_controlroom_p1(){
  ...
  // var_dump($alert_alias);die();
  // echo "<pre>";

  $data_site    = $this->m_dashboard_intervensi_postevent->data_site_bc_name($data_vehicle[0]['vehicle_site']);
  $data_site_ts = $this->m_dashboard_intervensi_postevent->master_site_bc_byName("ts_bc_master_site", $data_vehicle[0]['vehicle_site']);
...
function submit_intervention_controlroom_overspeed(){
  ...
  // echo "<pre>";
    ...
    $data_operator          = $this->m_dashboard_intervensi_postevent->check_data_karyawan_by_sid("ts_karyawan_beraucoal", $intervention_sid);
    $data_cr                = $this->m_dashboard_intervensi_postevent->check_data_karyawan_by_sid("ts_karyawan_beraucoal", $intervention_sid_cr);
  ...
  $update = $this->m_dashboard_intervensi_postevent->update_post_event($dbtableoverspeed, "overspeed_report_id", $alert_id, $data);
    if ($update) {
      $callback["error"]   = false;
      $callback["message"] = "Success Submit Intervention";
...
function dashboardintervention_2(){
  ...
  if ($privilegecode == 1) {
    $this->params["sidebar"]        = $this->load->view('newdashboard/partial/sidebar_superuser', $this->params, true);
    $this->params["content"]        = $this->load->view('newdashboard/dashboardberau/intervention/part_2/v_dashboard_intervention', $this->params, true);
    $this->load->view("newdashboard/partial/template_dashboard_superuser", $this->params);
  }elseif ($privilegecode == 2) {
    $this->params["sidebar"]        = $this->load->view('newdashboard/partial/sidebar_managementuser', $this->params, true);
    $this->params["content"]        = $this->load->view('newdashboard/dashboardberau/intervention/part_2/v_dashboard_intervention', $this->params, true);
...
// echo "<pre>";
  ...
  $this->params['content']   = $data_array_fix;
  $this->params['alarmtype'] = $alarmtype;
  $html                      = $this->load->view('newdashboard/dashboardberau/intervention/part_2/v_intervention_result', $this->params, true);
  $callback["html"]          = $html;
  $callback["report"]        = $data_array_fix;
...
function intervention_detail_controlroom(){
  ...
  if ($alarmtype == "Overspeed") {
    ...
    $this->params['year'] 			         = $year;
    $this->params['user_id_role'] 			 = $this->sess->user_id_role;
    $html                                = $this->load->view('newdashboard/dashboardberau/intervention/part_2/v_intervention_modal_overspeed', $this->params, true);
    $callback["report"]                  = $data_array_alert;
  }else {
    $reportdetail               = $this->m_securityevidence->getdetailreport_intervention($table, $alert_id, $alarm_report_imei, $sdate);
    $reportdetailvideo          = $this->m_securityevidence->getdetailreportvideo_intervention($table, $alert_id, $alarm_report_imei, $sdate);
    ...
    $this->params['year'] 			          = $year;
    $this->params['user_id_role'] 			  = $this->sess->user_id_role;
    $html                                 = $this->load->view('newdashboard/dashboardberau/intervention/part_2/v_intervention_modal', $this->params, true);
    $callback["report"]                   = $reportdetail;
  }
