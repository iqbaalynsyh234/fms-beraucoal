...
class Member extends Base {
	...
	function dologin($referer="", $lacakmobil="")
	{
		...
		//get data user ID
		$row = $this->get_SID_byusername($username);

		if(count($row)>0)
		{
			//jika lisensi beats maka get sync to portal
			if($row->user_license_id == "beats")
			{
				//DISABLE SEMENTARA HINGGA SERVER BEATS OK
				//$access_status = $this->sync_sid($username,$userpass);
				//if($access_status->code == "00")

				$access_status = $this->sync_sid_local($username,$userpass);
				if($access_status == "00")
				{
					//sukses
					//redirect ke platform (create session seperti biasa)
					$trackersite = "";
					...
					if ($trackersite)
					{
						...
						while(1)
						{
							...
							if ($total == 0)
							{
								...
								if($trackersite == "live.abditrack.com"){

									echo json_encode(array("error"=>false, "redirect"=>"http://live.abditrack.com/home/login/".$token));
									exit;
								}else{
				...
				{
					//disable sementara Hingga SERVER BEATS OK
					//$error_info = "Beats Response: ".$access_status->message.". Harap konfirmasi ke pihak Berau Coal.";
					$error_info = "Terjadi kesalahan atau SID belum terdaftar di FMS!";
					echo json_encode(array("error" => true, "message" => $error_info));
					return;
				}
		...
		//dimatikan dahulu sampai semua login gunakan SID
		/* else
		{
			$error_info = "SID Anda belum diintegrasikan ke platform FMS. Harap hubungi CS.";
			echo json_encode(array("error" => true, "message" => $error_info));
			return;
		...
		if($trackersite == ""){
			//$trackersite = $_SERVER['SERVER_NAME'];
			$trackersite = "localhost/fmsbc-jan2024";
		}
	...
	//Get Sync SID beats
	function get_SID_byusername($vusername)
	{

		$this->db->where("user_login", $vusername);
		$this->db->where("user_status", 1);
		$this->db->where("user_license_id", "beats");
		$this->db->join("company", "company_id = user_company", "left outer");
		$q = $this->db->get("user");
		$row = $q->row();
	...
	function sync_sid_local($vusername,$vpassname)
	{

		$code_respon = "99";
		if($vusername == $vpassname){
