...
//////////////////////////////////////////////
	...
	function get_overspeed_berecord($table,$dtvehicle,$sdate,$edate,$dtoverspeed)
	{
		$this->dbtensor = $this->load->database("tensor_report", true);
		$this->dbtensor->select("*");
		$this->dbtensor->where("overspeed_report_vehicle_device", $dtvehicle->vehicle_device);
		...
		$this->dbtensor->where("overspeed_report_level", $dtoverspeed->level_value);
	...
	function get_mdvr_berecord($table,$dtoperator,$sdate,$edate,$dtmdvr_cat,$companyid)
	{
		$this->dbtensor = $this->load->database("tensor_report", true);
		$this->dbtensor->select("*");
		//$this->dbtensor->where("alarm_report_status_sendhazard ", null); //default bukan nol
		$this->dbtensor->where("alarm_report_statusintervention_cr", 1);
		$this->dbtensor->where("alarm_report_truefalse_up", 1);
		$this->dbtensor->where("alarm_report_sid_cr", $dtoperator->karyawan_bc_sid);
		$this->dbtensor->where("alarm_report_fatiguecategory_cr", $dtmdvr_cat->level_alias);
		$this->dbtensor->where("alarm_report_vehicle_company",$companyid);
		$this->dbtensor->where("alarm_report_gpsstatus !=", "pushalert");
		$this->dbtensor->where("alarm_report_type",618); //only fatigue
		$this->dbtensor->where("alarm_report_start_time >=", $sdate);
		$this->dbtensor->where("alarm_report_start_time <=", $edate);
		$this->dbtensor->order_by("alarm_report_start_time", "DESC");
		$this->dbtensor->limit("5");
		$q = $this->dbtensor->get($table);
		$q        = $this->dbtensor->get($table);
	...
	function get_shift($table,$thishour)
	{
		$this->dbts = $this->load->database("webtracking_ts", true);
		$this->dbts->select("*");
	...
	function get_vehicle_bycompany($companyid)
	{
		$this->dbm = $this->load->database("default", true);
		$this->dbm->select("*");
		$this->dbm->where("vehicle_company", $companyid);
		$this->dbm->where("vehicle_status <>", 3);
		$this->dbm->order_by("vehicle_id", "ASC");
		$q        = $this->dbm->get("vehicle");
		$row = $q->result();
		return $row;
	}
	
	function get_master_speed_level($dbtable)
	{
	...
	function get_master_mdvr_level($dbtable)
	{
	...
	function calculate($companyid="",$dateselected="", $timeselected="")
	{
		date_default_timezone_set("Asia/Jakarta");
		$cronstartdate = date("Y-m-d H:i:s");
		...
		//$nowtime_wita  = date('Y-m-d H:i:s', strtotime($cronstartdate . '+1 hours'));
		$nowtime_wita  = date('Y-m-d H:i:s', strtotime($dateselected." ". $timeselected));
		
		print_r("===CRON BERECORD SEND Start WIB : ". $cronstartdate . "\r\n");
		print_r("===CRON BERECORD SEND Start WITA : ". $nowtime_wita . "\r\n");
		
	
		$this_hour = date("d",strtotime($nowtime_wita));
		$this_date = date("Y-m-d",strtotime($nowtime_wita));

		$date_periode = $nowtime_wita; 
		$this_shift = $this->get_shift("ts_hour_shift",$this_hour);
		
		$m1               = date("F", strtotime($date_periode)); 
		$year             = date("Y", strtotime($date_periode)); 
		$report           = "alarm_evidence_";
		$reportoverspeed  = "overspeed_hour_";
		$reportberecord  = "berecord_";
		$dbtable          = "";
		$dbtableoverspeed = "";
		$dbtableberecord = "";
		switch ($m1)
		{
			case "January":
						$dbtable = $report."januari_".$year;
						$dbtableoverspeed = $reportoverspeed."januari_".$year;
						$dbtableberecord = $reportberecord."januari_".$year;
			break;
			case "February":
						$dbtable = $report."februari_".$year;
						$dbtableoverspeed = $reportoverspeed."februari_".$year;
						$dbtableberecord = $reportberecord."februari_".$year;
			break;
			case "March":
						$dbtable = $report."maret_".$year;
						$dbtableoverspeed = $reportoverspeed."maret_".$year;
						$dbtableberecord = $reportberecord."maret_".$year;
			break;
			case "April":
						$dbtable = $report."april_".$year;
						$dbtableoverspeed = $reportoverspeed."april_".$year;
						$dbtableberecord = $reportberecord."april_".$year;
			break;
			case "May":
						$dbtable = $report."mei_".$year;
						$dbtableoverspeed = $reportoverspeed."mei_".$year;
						$dbtableberecord = $reportberecord."mei_".$year;
			break;
			case "June":
						$dbtable = $report."juni_".$year;
						$dbtableoverspeed = $reportoverspeed."juni_".$year;
						$dbtableberecord = $reportberecord."juni_".$year;
			break;
			case "July":
						$dbtable = $report."juli_".$year;
						$dbtableoverspeed = $reportoverspeed."juli_".$year;
						$dbtableberecord = $reportberecord."juli_".$year;
			break;
			case "August":
						$dbtable = $report."agustus_".$year;
						$dbtableoverspeed = $reportoverspeed."agustus_".$year;
						$dbtableberecord = $reportberecord."agustus_".$year;
			break;
			case "September":
						$dbtable = $report."september_".$year;
						$dbtableoverspeed = $reportoverspeed."september_".$year;
						$dbtableberecord = $reportberecord."september_".$year;
			break;
			case "October":
						$dbtable = $report."oktober_".$year;
						$dbtableoverspeed = $reportoverspeed."oktober_".$year;
						$dbtableberecord = $reportberecord."oktober_".$year;
			break;
			case "November":
						$dbtable = $report."november_".$year;
						$dbtableoverspeed = $reportoverspeed."november_".$year;
						$dbtableberecord = $reportberecord."november_".$year;
			break;
			case "December":
						$dbtable = $report."desember_".$year;
						$dbtableoverspeed = $reportoverspeed."desember_".$year;
						$dbtableberecord = $reportberecord."desember_".$year;
			break;
		}
		
		//master data 
		$alert_mdvr_type = array("618"); //fatigue only trial
		$master_operator =  $this->get_data_operator($companyid);
		$master_vehicle = $this->get_vehicle_bycompany($companyid);
		$master_company = $this->get_company_beats($companyid);
		$master_speed_level = $this->get_master_speed_level("ts_speed_level");
		$master_mdvr_level = $this->get_master_mdvr_level("ts_mdvr_level"); 
		
		if($this_shift == 1)
		{
			$sdate = date("Y-m-d H:i:s", strtotime($this_date."06:00:00"));
			$edate = $nowtime_wita;
		}
		
		printf("===GET OVERSPEED PERIODE %s %s \r\n", $sdate, $edate); 
		
		$this->dbreport = $this->load->database("tensor_report", true);
		
		//$total_vehicle = count($master_vehicle);
		$total_operator = count($master_operator);
		$total_vehicle = 0;
		//$total_operator = 0;
		if($total_vehicle > 0)
		{
			...
			{
				printf("===GET VEHICLE %s %s %s (%s/%s) \r\n", $master_vehicle[$i]->vehicle_no, $sdate, $edate, $i+1, $total_vehicle); 
				$total_alert = 0;
				for ($x=0; $x < count($master_speed_level); $x++) 
				{
					$total_alert = $this->get_overspeed_berecord($dbtableoverspeed,$master_vehicle[$i],$sdate,$edate,$master_speed_level[$x]);
					
					printf("===RESULT Alert : %s ,total %s \r\n", $master_speed_level[$x]->level_name,$total_alert); 
					//prepare to insert table:
					unset($datainsert);
					$datainsert["berecord_operator_sid"] = "";
					$datainsert["berecord_operator_id_sync"] = "";
					$datainsert["berecord_operator_name"] = "";
					$datainsert["berecord_vehicle_id"] = $master_vehicle[$i]->vehicle_id;
					$datainsert["berecord_vehicle_no"] = $master_vehicle[$i]->vehicle_no;
					$datainsert["berecord_vehicle_name"] = $master_vehicle[$i]->vehicle_name;
					$datainsert["berecord_vehicle_company"] = $master_vehicle[$i]->vehicle_company;
					$datainsert["berecord_vehicle_device"] = $master_vehicle[$i]->vehicle_device;
					$datainsert["berecord_vehicle_cam"] = $master_vehicle[$i]->vehicle_mv03;
					$datainsert["berecord_mode"]= "overspeed";
					$datainsert["berecord_alert_id"] = "1";
					$datainsert["berecord_alert_name"] = "Overspeed";
					$datainsert["berecord_alert_name_detail"] = $master_speed_level[$x]->level_name;
					$datainsert["berecord_shift"] = $this_shift;
					$datainsert["berecord_week"] = 1; 
					$datainsert["berecord_date"]= $this_date;
					$datainsert["berecord_last_update"] = date("Y-m-d H:i:s");
					$datainsert["berecord_score"] = $master_speed_level[$x]->level_sanksi_lubang;
					$datainsert["berecord_status"] = 1; //default
					$datainsert["berecord_sanksi"] = $master_speed_level[$x]->level_sanksi_detail;
					$datainsert["berecord_remark"] = $master_speed_level[$x]->level_remark;
					$datainsert["berecord_total"] = $total_alert;
					if($total_alert > 0){
						$this->dbreport->insert($dbtableberecord,$datainsert);
						printf("===INSERT BERECORD OVERSPEED=== \r\n");	
					}
		...
		if($total_operator > 0)
		{
			...
			{
				printf("===GET OPERATOR %s %s %s (%s/%s) \r\n", $master_operator[$i]->karyawan_bc_name, $sdate, $edate, $i+1, $total_operator); 
				$total_alert = 0;
				for ($x=0; $x < count($master_mdvr_level); $x++) 
				{
					$total_alert = $this->get_mdvr_berecord($dbtable,$master_operator[$i],$sdate,$edate,$master_mdvr_level[$x],$companyid);
					
					printf("===RESULT Alert : %s ,total %s \r\n", $master_mdvr_level[$x]->level_name,$total_alert); 
					//prepare to insert table:
					unset($datainsert);
					$datainsert["berecord_operator_sid"] = $master_operator[$i]->karyawan_bc_sid;
					$datainsert["berecord_operator_id_sync"] = $master_operator[$i]->karyawan_bc_id_sync;
					$datainsert["berecord_operator_name"] = $master_operator[$i]->karyawan_bc_name;
					$datainsert["berecord_vehicle_id"] = "";
					$datainsert["berecord_vehicle_no"] = "";
					$datainsert["berecord_vehicle_name"] = "";
					$datainsert["berecord_vehicle_company"] = "";
					$datainsert["berecord_vehicle_device"] ="";
					$datainsert["berecord_vehicle_cam"] = "";
					$datainsert["berecord_mode"]= "fatigue";
					$datainsert["berecord_alert_id"] = "1";
					$datainsert["berecord_alert_name"] = "Fatigue";
					$datainsert["berecord_alert_name_detail"] = $master_mdvr_level[$x]->level_name;
					$datainsert["berecord_shift"] = $this_shift;
					$datainsert["berecord_week"] = 1; 
					$datainsert["berecord_date"]= $this_date;
					$datainsert["berecord_last_update"] = date("Y-m-d H:i:s");
					$datainsert["berecord_score"] = $master_mdvr_level[$x]->level_sanksi_lubang;
					$datainsert["berecord_status"] = 1; //default
					$datainsert["berecord_sanksi"] = $master_mdvr_level[$x]->level_sanksi_detail;
					$datainsert["berecord_remark"] = $master_mdvr_level[$x]->level_remark;
					$datainsert["berecord_total"] = $total_alert;
					if($total_alert > 0){
